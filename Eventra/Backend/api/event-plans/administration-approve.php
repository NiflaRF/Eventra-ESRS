...
if (!empty($data->event_plan_id) && !empty($data->action)) {
    ...
    if ($signedLetter->create()) {
        ...
        $notification->createEventPlanNotification(
            ...
            $data->action === 'approve' 
                ? "University Administration has approved your event plan '{$eventPlan->title}' and sent signed document to Super-Admin."
                : "Your event plan '{$eventPlan->title}' has been rejected by University Administration.",
        ...
        if (!empty($superAdminUsers)) {
            foreach ($superAdminUsers as $superAdmin) {
                $notification->createEventPlanNotification(
                    $superAdmin['id'],
                    $data->action === 'approve' ? 'University Administration Approval Received' : 'Event Plan Rejected by Administration',
                    $data->action === 'approve'
                        ? "University Administration has approved event plan '{$eventPlan->title}' and sent signed document."
                        : "University Administration has rejected event plan '{$eventPlan->title}'.",
